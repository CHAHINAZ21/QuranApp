...
export default function HizbDetail() {
  ...
  const { hizbQuarter } = useLocalSearchParams();
  const currentQuarter = parseInt(hizbQuarter as string) || 1;
  ...
  useEffect(() => {
    const hizbInfo = hizbQuarters.find((h) => h.hizbQuarter === currentQuarter);
  ...
  const goToHizbQuarter = (newHizb: number) => {
    router.push(`/HizbQuarterDetail?hizbQuarter=${newHizb}`);
  };

  const goToDetailAyah = (ayah: number, surah: number, text: string, surahName: string) => {
   router.push({
        pathname: '/DetailAyah',
        params: {
          ayah: ayah.toString(),
  ...
  const playAudio = async (surah: number, ayah: number) => {
    try {
      ...
      const uri = `https://everyayah.com/data/Ghamadi_40kbps/${paddedSurah}${paddedAyah}.mp3`;
  ...
  return (
    <View style={styles.container}>
      <View style={styles.navHeader}>
              <TouchableOpacity onPress={() => goToHizbQuarter(currentQuarter+ 1)} disabled={currentQuarter >= 240}>
                <Ionicons
                  name="arrow-back-circle"
                  size={40}
                  color={currentQuarter>= 240 ? '#ccc' : '#5e2a84'}
                />
              </TouchableOpacity>
      
              <Text style={styles.title}>{title}</Text>
      
              <TouchableOpacity onPress={() => goToHizbQuarter(currentQuarter- 1)} disabled={currentQuarter <= 1 }>
                <Ionicons
                  name="arrow-forward-circle"
                  size={40}
                  color={currentQuarter <= 1 ? '#ccc' : '#5e2a84'}
                />
              </TouchableOpacity>
            </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity onPress={() => goToDetailAyah(item.ayah, item.surah, item.text, item.surahName)}>
